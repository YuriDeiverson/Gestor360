    ...
    <>
      ...
      <tr className="sm:hidden">
        <td colSpan={7} className="p-0">
          <div className="p-4 border-b border-gray-100 hover:bg-gray-50 transition space-y-2">
            <div className="flex justify-between">
              ...
              <span
                className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${statusBadge}`}
              >
                {transaction.status === "completed" ? "Pago" : "Pendente"}
              </span>
            </div>
            <div className="text-sm text-gray-600 flex flex-col gap-0.5">
              ...
              <div>Conta: {transaction.method === "Cartão de Crédito" ? (transaction.cardName || "Cartão") : transaction.account}</div>
              <div>Cartão: {transaction.cardName || "-"}</div>
              <div>
                Valor:{" "}
            ...
            {canPayNextInstallment && handlePayInstallment && (
              <button
                onClick={() => handlePayInstallment(transaction)}
                className="w-full mt-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-sm hover:shadow-md active:scale-95"
              >
                Pagar Parcela
              </button>
            )}
